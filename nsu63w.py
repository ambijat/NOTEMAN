...
def opennote():
    ...
    if gg > 4:
        ...
        messagebox.showinfo(title="PROCEDURAL ERROR", message="No File Created.")
...
def tesspath():
    ft = [('exe files', '.exe')]
    global tessloc
    tessloc = filedialog.askopenfilename(parent=root, initialdir=os.getcwd(),
                                         title="Please select a file:", filetypes=ft).replace('\n', '')
    if len(tessloc) > 2:
        messagebox.showinfo(title="TASK DONE", message="SUCCESS! TESSERACT\nLocation Set.")
    else:
...
pbtn1 = Button(f0, text="PASTE", activebackground="green", activeforeground="red",
               bd="3", bg="green", command=paste, fg="white", font=('arial', 10, 'bold'))
...
pbtn6 = Button(f0, text="CLP_OCR", activebackground="yellow",  activeforeground="RoyalBlue3",
               bd="3", bg="blue", command=clpocr, fg="white", font=('arial', 10, 'bold'))
pbtn6.grid(row=11, column=0, sticky='NSEW')

pbtn88 = Button(f0, text="TessLoc", activebackground="pink", activeforeground="SeaGreen",
                bd="2", bg="powder blue", command=tesspath, fg="purple", font=('arial', 10, 'bold'))
pbtn88.grid(row=12, column=0, sticky='NSEW')
#===================COLUMN1
